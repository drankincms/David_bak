...
float mx[25],y[25],y0lo[25],y0hi[25],y1lo[25],y1hi[25];
 ...
 for (int il=0; il<11; il++){
   for (int ir=0; ir<11; ir++){
     ...
     numal=il;
     numar=ir;
     sprintf(filename, "%s%d%s%d%s", base, numal,mid,numar,"/bayesian_limits_expected.txt");
       cout << filename << endl;
       num++;
	 ...
	 // cout<<mx[i]<<" "<<y[i]<<" "<<y0lo[i]<<" "<<y0hi[i]<<" "<<y1lo[i]<<" "<<y1hi[i]<<endl;
	 if (i<21) { // 21 mass values
	 explim[il][ir][i]=y[i];
	 x[il][ir][i]=y[i];
	 }
	 i++;
 ...
 for (int k=0; k<11; k++) {
   for (int j=0; j<11; j++) {
	 ...
	 cout << "al/ar/ mass "<< " " << aal << " " << aar << " " << m[im] << " " << x[k][j][im]<< " " << t[k][j][im]<< " " << st_wgt << " " << rt_wgt << " " << lt_wgt << " " << mt_wgt <<endl; 
...
c1->Update();
c1->Print("contour_arVSal-combined_expected.pdf");
c1->Print("contour_arVSal-combined_expected.png");
