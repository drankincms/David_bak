...
def get_model():
    ...
    model = build_model_from_rootfile('/uscms_data/d2/dsperka/8TeV/MakeTBntuples/29Jan/CMSSW_5_3_6/src/UserCode/dsperka/wprimetb/RootFiles_For2DLimits_12Feb_finalbins/electron_BestJetJet2W_M_WprimeRight_Histos-final_85_140_dr03_lep50.root',include_mc_uncertainties=True)
    ...
    model.add_lognormal_uncertainty('elepeff', math.log(1.01), '*','elec_invmass_final')
    model.add_lognormal_uncertainty('ehlteff', math.log(1.02), '*','elec_invmass_final')
...
#plot_exp, plot_obs = bayesian_limits(model, signal_processes = [['wp800'],['wp2000']])
...
plot_exp.write_txt('elec_bayesian_limits_expected.txt')
plot_obs.write_txt('elec_bayesian_limits_observed.txt')
...
parVals = ml_fit(model, input = 'data', signal_prior = 'flat', signal_processes = [['wp900'],['wp2000']], nuisance_parameters = '' )
#parVals = ml_fit(model, input = 'data', signal_prior = 'flat', nuisance_constraint = '')
print parVals
...
report.write_html('htmlout_elec')
...
#parVals = ml_fit(model, input = 'data', signal_prior = 'flat', nuisance_constraint = '')
#parVals = ml_fit(model, input = 'data', nuisance_constraint ='')
#parVals = ml_fit2 (   model, input = 'data', n = 1, signal_prior = 'flat', nuisance_constraint = '',) 
#print parVals
#parVals = ml_fit_coefficients(model)
#print parVals
