...
files={}
files["elec"]="RootFiles_For2DLimits_23Sep/electron_BestJetJet2W_M_WprimeRight_Histos-onebtags_85_140_dr03_lep50.root"
files["mu"]="RootFiles_For2DLimits_23Sep/muon_BestJetJet2W_M_WprimeRight_Histos-onebtags_85_140_dr03_lep50.root"
...
for channel in ["elec","mu"]:
    ...
    w=file.Get(channel+"_invmass_onebtags__wjets")
    tt=file.Get(channel+"_invmass_onebtags__ttbar")
    ...
    for sys in ["jes","jer","btag","PU","matching","q2scale","zerotagshape","hf",'shape']:
        ...
        wp=file.Get(channel+"_invmass_onebtags__wjets__"+sys+"__plus")
        wm=file.Get(channel+"_invmass_onebtags__wjets__"+sys+"__minus")
        if not wp:
            wp=file.Get(channel+"_invmass_onebtags__wjets")
            wm=file.Get(channel+"_invmass_onebtags__wjets")
        bp.Add(wp)
        bm.Add(wm)
        
        ttp=file.Get(channel+"_invmass_onebtags__ttbar__"+sys+"__plus")
        ttm=file.Get(channel+"_invmass_onebtags__ttbar__"+sys+"__minus")
        if not ttp:
            ttp=file.Get(channel+"_invmass_onebtags__ttbar")
            ttm=file.Get(channel+"_invmass_onebtags__ttbar")
        bp.Add(ttp)
        bm.Add(ttm)
        ...
        if (channel == 'elec'):
            latex3.DrawLatex(0.9, 0.80, "e+jets N_{b-tags} = 1");
        if (channel == 'mu'):
            latex3.DrawLatex(0.9, 0.80, "#mu+jets N_{b-tags} = 1");
        ...
        c.SaveAs(channel+"_"+sys+"_1btags.pdf")
